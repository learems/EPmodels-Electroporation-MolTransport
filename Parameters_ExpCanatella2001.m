...
% Time stepping
% par.tlist1 = '10^{range(-10,(log10(2e2)+10)/600,log10(2e2))}';
% par.tlist2 = '10^{range(-10,(log10(2e2)+10)/200,log10(2e2))}';
par.tlist1 = '10^{range(-10,(log10(tpulse[1/s])+10)/200,log10(tpulse[1/s]))} tpulse+10^{range(-8,(log10(1e-4)+8)/50,log10(1e-4))} tpulse+10^{range(log10(1e-4),(log10(2e2)-log10(1e-4))/350, log10(2e2))}';
par.tlist2 = '10^{range(-10,(log10(tpulse[1/s])+10)/40,log10(tpulse[1/s]))}  tpulse+10^{range(-8,(log10(1e-4)+8)/5,log10(1e-4))}  tpulse+10^{range(log10(1e-4),(log10(2e2)-log10(1e-4))/50, log10(2e2))}';

% Pulse parameters and temperature
par.pulseshape = 'exponential'; % Pulse shape
par.Eapp = 1e5; % Applied electric field [V/m]
par.tpulse = 1e-3; % Pulse duration [s]
par.trise = 1e-8; % Pulse rise time [s]
...
par.sigma_e = 1.29; % External conductivity [S/m]
...
par.c1_e0 = 0.01; % Initial extracellular concentration of solute 1 [mol/m^3]
